#!/usr/bin/env bun
// clickzoom — record screen + clicks + mic, render polished MP4 with auto-zoom on each click.
//
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--duration" || a === "-t") args.duration = Number(argv[++i]);
      ...
      console.log(`clickzoom — record a window + render polished MP4

Usage: clickzoom [options]
...
async function record(args: Args, windowID: number, outputPath: string, workDir: string) {
  ...
  console.error(`[clickzoom] starting input recorder…`);
  ...
  console.error(`\n[clickzoom] >>> RECORDING ${args.duration}s <<<\n`);
  ...
  if (recorderProc.pid !== undefined) {
    console.log(`[clickzoom] recorder PID: ${recorderProc.pid}`);
  }
...
async function main() {
  ...
  if (!existsSync(CLICKS_BIN)) {
    console.error(`[clickzoom] clicks binary missing at ${CLICKS_BIN} — run \`pnpm run build:clicks\``);
    process.exit(1);
  }
  if (!existsSync(RECORDER_BIN)) {
    console.error(`[clickzoom] recorder binary missing at ${RECORDER_BIN} — run \`pnpm run build:recorder\``);
    process.exit(1);
  }
  if (!existsSync(WINDOWS_BIN)) {
    console.error(`[clickzoom] windows binary missing at ${WINDOWS_BIN} — run \`pnpm run build:windows\``);
    process.exit(1);
  }
  ...
  console.error(`[clickzoom] target window: "${windowInfo.title}" (${windowInfo.app}) ${windowInfo.bounds.w}x${windowInfo.bounds.h} @ (${windowInfo.bounds.x}, ${windowInfo.bounds.y})`);

  if (args.skipRecord) {
    console.error(`[clickzoom] --skip-record: no longer supported in v3 (no intermediate stage to reuse). Re-record.`);
    process.exit(1);
  }
  ...
  console.error(`[clickzoom] ${committed.length}/${clicks.length} click(s) committed, ${cursor.length} cursor sample(s) (overlay TBD)`);
  console.error(`[clickzoom] wrote ${outputPath}`);
}

main().catch((err) => {
  console.error(`[clickzoom] error: ${err.message}`);
  process.exit(1);
});
